...
const SidebarItem = React.forwardRef<HTMLAnchorElement, SidebarItemProps>(
    ({ className, icon: Icon, label, isActive, isCollapsed, ...props }, ref) => {
        return (
            <a
                ref={ref}
                className={cn(
                    "flex items-center gap-4 px-6 py-3 transition-colors border-l-4",
                    isActive
                        ? "bg-[#363a7d] border-brand-woodwork text-white"
                        : "border-transparent text-gray-300 hover:bg-[#363a7d] hover:border-white/20 hover:text-white",
