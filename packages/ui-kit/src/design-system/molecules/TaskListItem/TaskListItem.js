...
const TaskListItem = React.forwardRef(({ className, title, description, area, urgency, duration, status, onClickUpLink, onClaim, isNew, ...props }, ref) => {
    const Icon = getAreaIcon(area);
    return (<div ref={ref} className={cn("group flex flex-col md:flex-row bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all hover:border-brand-purple/30", className)} {...props}>
                <div className={cn("hidden md:block w-1.5 shrink-0", status === 'ready' ? "bg-brand-textiles" : "bg-brand-purple")}/>
                ...
                <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-3 shrink-0 p-5 md:p-6 md:pl-0 border-t md:border-t-0 md:border-l border-gray-100 bg-gray-50/50 md:bg-transparent">
                    <Button variant="secondary" size="sm" className="w-full md:w-auto shadow-sm whitespace-nowrap font-bold uppercase tracking-wider" onClick={onClaim}>
                        Claim Task
                    </Button>

                    {onClickUpLink && (<a href={onClickUpLink} target="_blank" rel="noopener noreferrer" className="hidden md:flex items-center gap-1 text-[11px] font-medium uppercase tracking-wide text-slate-400 hover:text-brand-purple transition-colors">
