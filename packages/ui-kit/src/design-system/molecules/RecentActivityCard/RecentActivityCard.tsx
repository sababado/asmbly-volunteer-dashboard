...
const RecentActivityCard = React.forwardRef<HTMLDivElement, RecentActivityCardProps>(
    ({ className, activities, onViewAll, ...props }, ref) => {
        return (
            <div
                ref={ref}
                className={cn("flex flex-col bg-white border border-gray-200 shadow-sm", className)}
                {...props}
            >
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    ...
                    <button
                        onClick={onViewAll}
                        className="text-xs text-brand-purple font-bold uppercase tracking-wider hover:underline"
                    >
                        View All
                    </button>
                </div>
