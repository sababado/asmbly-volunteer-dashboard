...
async def clickup_webhook(payload: ClickUpWebhookPayload):
    ...
    """
    try:
        logger.info(f"Received ClickUp webhook: {payload.model_dump_json(exclude_none=True)}")
        
        # Determine the event type and process accordingly
        # ClickUp sends 'event' field like 'taskCreated', 'taskUpdated', etc.
        # We are mainly interested in task creation or updates that might represent a problem report.
        ...
        # Startups often want 200 even on error to prevent webhook retries from the provider if it's a logic error
        # But for development visibility, let's return 500 or 400 if strictly necessary.
        # ClickUp will retry on 500. raising HTTPException will likely result in 500 or 422.
